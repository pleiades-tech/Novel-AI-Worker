...
import json
import pathlib
from tenacity import retry, stop_ater_attempt, wait_fixed
from google import genai
from google.genai import types
from pydantic import BaseModel
from types import NovelChapter, NovelDialogue

client = genai.Client(api_key=os.getenv('GEMINI_KEY'))

@retry(stop=stop_ater_attempt(3), wait=wait_fixed(2))
def extract_chapter_from_pdf(path):
...
@retry(stop=stop_ater_attempt(3), wait=wait_fixed(2))
def extract_dialogue_from_pdf(path):
  filepath = pathlib.Path(path)
  prompt = """
